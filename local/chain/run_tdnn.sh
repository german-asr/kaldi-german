...
graph_dir=$dir/graph_ng6
if [ $stage -le 16 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 --remove-oov data/lang_test_ng6 $dir $graph_dir
fi
...
if [ $stage -le 17 ]; then
  ...
  for decode_set in dev_cv test_cv dev_tuda test_tuda; do
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          ...
          $graph_dir data/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_ng6 || exit 1
  done
  wait
  if [ -f $dir/.error ]; then
